...
const isLocalUploadUrl = (value?: string | null): value is string =>
  Boolean(value && value.includes("/uploads/"));

const toLocalFilePath = (value: string): string | null => {
...
const migrateTenantInvoiceLogos = async (): Promise<number> => {
  ...
  for (const tenant of tenants) {
    if (!isLocalUploadUrl(tenant.invoiceLogoUrl)) continue;
...
const migrateBanners = async (): Promise<number> => {
...
const migrateCommunityMedia = async (): Promise<number> => {
  ...
  for (const post of posts) {
    ...
    if (!filePath) continue;
    const resourceType =
      post.mediaType === CommunityMediaType.VIDEO ? ("video" as const) : ("image" as const);
    const uploaded = await uploadLocalFile(
      filePath,
      `tenants/${post.tenantId}/community`,
      resourceType
    );
    if (!uploaded) continue;
...
const migrateMenuItems = async (): Promise<number> => {
...
const migrateCategories = async (): Promise<number> => {
  ...
  for (const category of categories) {
    ...
    const uploaded = await uploadLocalFile(
      filePath,
      `tenants/${category.tenantId}/menu/categories`,
      "image"
    );
    if (!uploaded) continue;
