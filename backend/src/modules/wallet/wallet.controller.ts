...
const topupEligibleRoles = new Set<Role>([Role.TEACHER, Role.STAFF]);
...
export const topupMyWallet = async (req: Request, res: Response): Promise<void> => {
  const tenantId = req.tenantId as string;
  const userId = req.user?.sub;
  if (!userId) {
    throw new AppError("Unauthorized", 401);
  ...
  const { amount, upiReference, note } = req.body as {
    amount?: number;
    upiReference?: string;
    note?: string;
  };
