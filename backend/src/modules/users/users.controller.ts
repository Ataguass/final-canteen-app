...
const parseManageableRole = (value: string | undefined): Role | null => {
...
export const updateMyProfile = async (req: Request, res: Response): Promise<void> => {
  ...
  const { name, phone, email, rollNumber } = req.body as {
    name?: string;
    phone?: string;
    email?: string;
    rollNumber?: string;
  ...
  const nextRollNumber =
    rollNumber !== undefined ? (rollNumber.trim() ? rollNumber.trim() : null) : current.rollNumber;
...
export const createUser = async (req: Request, res: Response): Promise<void> => {
  const tenantId = req.tenantId as string;
  const { name, phone, password, role, isApproved = true } = req.body as {
    name: string;
    phone: string;
    password: string;
    role: Role;
...
export const bulkImportUsers = async (req: Request, res: Response): Promise<void> => {
  const tenantId = req.tenantId as string;
  const { rows } = req.body as { rows?: BulkImportUserInput[] };
  ...
  const inputPhoneSet = new Set<string>();
  ...
  const existingPhoneSet = new Set(existingUsers.map((item) => item.phone).filter(Boolean) as string[]);
  const createdUsers: ReturnType<typeof toPublicUser>[] = [];
...
export const superUpdateAdmin = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
...
export const superToggleAdminStatus = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const { isActive } = req.body as { isActive?: boolean };
  if (typeof isActive !== "boolean") throw new AppError("isActive is required", 400);
...
export const superDeleteAdmin = async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const user = await prisma.user.findFirst({ where: { id, role: Role.ADMIN } });
  if (!user) throw new AppError("Admin not found", 404);
