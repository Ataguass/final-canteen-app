...
const buildSingleFileZip = (entryFileName, content, modifiedAt) => {
...
const getTenantBackupDir = (tenantId) => path.join(backupRootDir, tenantId);
const ensureTenantBackupDir = async (tenantId) => {
...
const readBackupFile = async (tenantId, backupIdRaw) => {
...
const serializeBackupPayload = (payload) => `${JSON.stringify(payload, null, 2)}\n`;
const buildCounts = (backup) => ({
...
export const createBackup = async (req, res) => {
    const tenantId = req.tenantId;
    const [tenant, users, categories, menuItems, banners, communityPosts, orders, orderItems, stockMovements, walletTransactions] = await Promise.all([
...
export const restoreBackup = async (req, res) => {
    ...
    if (!backup.users.some((entry) => entry.id === requesterUserId && (entry.role === Role.ADMIN || entry.role === Role.SUPER_ADMIN))) {
        throw new AppError("Safety check failed: this backup does not contain your current admin account. Use a backup created after your account was created.", 400);
    }
    ...
    const validOrderItems = backup.orderItems.filter((entry) => orderIds.has(entry.orderId) && menuItemIds.has(entry.menuItemId));
